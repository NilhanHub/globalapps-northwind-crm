# Northwind desktop-agent access
...
Supported collections are companies, people, routes and activities. Archive and restore use `POST /api/{companies|people|routes}/:id/{archive|restore}`. Archive requires a reason and retains history. Stage/outcome changes use `POST /api/routes/:id/actions`, not raw route patches. Won and Dead require a reason. Bulk scheduling uses `POST /api/routes/bulk/actions`.
