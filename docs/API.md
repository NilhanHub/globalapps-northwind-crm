...
Operational reads: `GET /api/live`, `/api/ready`, `/api/workspace/revision` and authenticated `/api/diagnostics`. Health responses expose release version, commit, build time, repository type, sanitized Firestore database ID, application readiness and a sanitized backup state/count/age summary. Production commit and build time come only from clean-checkout metadata embedded in the compiled API; production fails closed if that identity is absent or invalid. Backup warnings remain advisory so a recovery-system failure cannot take the CRM itself offline.
