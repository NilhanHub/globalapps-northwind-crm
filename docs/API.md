...
Paginated reads use opaque query-bound cursors and return `{ items, nextCursor, hasMore }`: `GET /api/companies/page`, `/api/people/page`, `/api/routes/page` and `/api/activities/page`. The default page size is 50 and the maximum is 100. Companies and People accept normalized `q` input. Routes accept `q`, `ownerId` and `view=all|unassigned|overdue|unscheduled|imported`; these filters are bound into the opaque cursor. `GET /api/search` performs normalized prefix search and `GET /api/routes/metrics` derives workspace-wide metrics independently of the loaded page.

Operational reads: `GET /api/live`, `/api/ready`, `/api/workspace/revision` and authenticated `/api/diagnostics`. Health responses expose release version, commit, build time, repository type, sanitized Firestore database ID, application readiness and a sanitized backup state/count/age summary. Production commit and build time come only from verified build metadata embedded in the compiled API; Hostinger lockfile normalization is the sole permitted tracked drift, and production fails closed if any other drift or invalid identity is detected. Backup warnings remain advisory so a recovery-system failure cannot take the CRM itself offline.
